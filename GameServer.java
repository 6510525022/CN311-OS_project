...
public class GameServer {
    public static void main(String[] args) {
        final int port = 1234;
        GameState gameState = new GameState(); // ✅ ใช้ shared state
        ...
        try (ServerSocket serverSocket = new ServerSocket(port)) {
            ...
            while (true) {
                ...
                // ✅ ส่ง GameState แทน primitive int
...
class ClientHandler extends Thread {
    ...
    public void run() {
        try (
            ...
            writer.println("เชื่อมต่อกับ Server สำเร็จ" + "END");

            String text;
            while ((text = reader.readLine()) != null) {
                int currentPhase = gameState.getGamePhase(); // ✅ ดึงค่าแบบ real-time
